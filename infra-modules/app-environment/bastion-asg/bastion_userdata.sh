...
aws s3 cp s3://rancher-config-test/cluster.yml ./
aws s3 cp s3://rancher-config-test/ec2-ssh-key.pem ./
...
aws secretsmanager create-secret --name rkekubeconfig --description "Kube config for RKE cluster" --secret-string "$KUBE_CONFIG" --region us-east-1
